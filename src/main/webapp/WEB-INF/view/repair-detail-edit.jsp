					...
					<div class="card">
						<div class="card-header">
							<h3 class="card-title">Repair Detail Of Repair Order No:
								${repairOrder.code} , Model : ${modelRepair.name} , Rank:
								${rank.name}</h3>
						</div>
						<div class="card-header">
							<h3 class="card-title">
							...
							<input type="hidden" id="modelId" value="${modelRepair.id}" /> <input
								type="hidden" id="rankId" value="${rank.id}" /> <input
						...
						<div class="card-body">
							<table id="data-table" class="table table-bordered">
								...
								<tbody>
									<c:choose>
										<c:when test="${repairOrder.repairRank.name == rank.name}">
											<c:forEach var="repairDetail" items="${repairDetailList}">
												<tr>
													<td>${repairDetail.part.code}</td>
													<td>${repairDetail.part.name}</td>
													<td><input data-id="${repairDetail.part.id}"
														class="quantity" type="number" value="${repairDetail.quantity}"
														min="0" step="1" /></td>
												</tr>
											</c:forEach>
										</c:when>
										<c:otherwise>
											<c:forEach var="partDTO" items="${partQuantityList}">
												<tr>
													...
													<td><input data-id="${partDTO.part.id}"
														class="quantity" type="number" value="${partDTO.quantity}"
														min="0" step="1" /></td>
												</tr>
											</c:forEach>
										</c:otherwise>
